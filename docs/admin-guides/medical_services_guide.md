...
2. Заполните поля:
   - **Тип учреждения**: Выберите тип (Клиника/Реабилитационный центр)
...
- **Тип учреждения**: Ограничен только клиниками и реабилитационными центрами
...
- Для `ServiceCategory` и `TherapyMethod`: slug генерируется автоматически из названия
...
- Поддерживаются клиники (`Clinic`) и реабилитационные центры (`RehabCenter`)
...
2. **Указывайте точную длительность** для планирования
3. **Назначайте специалистов** для персонального подхода
...
3. Проверьте корректность **длительности** (должна быть > 0)
...
2. **Добавьте услуги**:
   ...
   - Консультация нарколога (60 мин)
   - Детоксикация (120 мин)
   - Кодирование (90 мин)
