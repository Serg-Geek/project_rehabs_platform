...
class RecoveryStoryAdmin(admin.ModelAdmin):
    list_display = ('title', 'category', 'author', 'content_type', 'object_id', 'is_published', 'publish_date')
    list_filter = ('category', 'is_published', 'content_type')
    search_fields = ('title', 'content', 'author')
    prepopulated_fields = {'slug': ('title',)}
    inlines = [RecoveryStoryImageInline]
    date_hierarchy = 'publish_date'
    fieldsets = (
        (None, {
            'fields': ('title', 'slug', 'category', 'author', 'content_type', 'object_id', 'content', 'excerpt', 'image', 'is_published', 'publish_date')
        }),
        ('SEO', {
