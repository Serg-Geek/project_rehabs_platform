    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for city_name, (lat, lng) in cities_data.items():
                ...
                if city:
                    # Создаем или обновляем координаты
                    ...
                    if created:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f'✅ Созданы координаты для {city_name}: {lat}, {lng}'
                            )
                        )
                    else:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'\n🎉 Операция завершена!\n'
                f'✅ Создано: {created_count} записей\n'
                f'🔄 Обновлено: {updated_count} записей\n'
                f'📊 Всего городов с координатами: {CityCoordinates.objects.count()}'
            )
        ) 