class FormHandler {
    ...
    validatePhone(e) {
        ...
        if (!phoneRegex.test(phone)) {
            this.phoneError.textContent = 'Введите номер в формате: +7 (XXX) XXX-XX-XX';
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                // Ошибка
                this.errorMessage.textContent = data.error || 'Произошла ошибка при отправке формы. Пожалуйста, попробуйте позже.';
                this.errorModal.show();
            }
        } catch (error) {
            this.errorMessage.textContent = 'Произошла ошибка при отправке формы. Пожалуйста, попробуйте позже.';
            this.errorModal.show();
        } finally {
